...
st.subheader("Évolution des 5 pathologies les plus fréquentes entre 2015-2023 dans le depertement Haute-Garonne 31")
...
ax.set_title("Évolution de la prévalence des 5 pathologies les plus fréquentes (Département 31)")
ax.set_xlabel("Année")
...
# Filtrage de base pour le 66 et hors_patho
data_patho = data[~data['patho_niv1'].str.contains('|'.join(re.escape(x) for x in hors_patho), case=False, na=False, regex=True)]
...
# On ne garde que ces 5 pathologies pour la suite
df_top5_66 = data_patho[data_patho['patho_niv1'].isin(top_5_names)]
...
ax.set_title("Évolution de la prévalence des 5 pathologies les plus fréquentes (Département 66)")
ax.set_xlabel("Année")
ax.set_ylabel("Prévalence calculée")
ax.legend(title="Pathologie")
ax.grid(True, alpha=0.5)
